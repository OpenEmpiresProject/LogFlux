...
void MiniMap::paintEvent(QPaintEvent*)
{
	...
	for (const auto& m : m_markers)
	{
		...
		switch (m.type)
		{
		case Bookmark: color = QColor(80, 180, 255); break; // bright blue
		case Warning:  color = QColor(255, 253, 85); break; // orange
		case Error:    color = QColor(220, 50, 50); break; // red
		default: continue;
		}
		...
		if (blockNum >= totalBlocks) blockNum = totalBlocks - 1;
