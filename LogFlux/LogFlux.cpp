...
LogFlux::LogFlux(QWidget* parent) : QMainWindow(parent)
{
    ...
    QFont font;
    font.setFamilies(
        {"Consolas", "Courier New", "Monaco", "Menlo", "DejaVu Sans Mono", "Monospace"});
    ...
    connect(ui.plainTextEdit, &QPlainTextEdit::cursorPositionChanged, this,
            &LogFlux::onCursorPositionChanged);
    ...
    connect(ui.bookmarkBar, &BookmarkArea::bookmarkToggled, ui.scrollbarMinimap,
            &MiniMap::setBookmark);
...
{
    const QString path =
        QFileDialog::getOpenFileName(this, tr("Select log file"), QString(), tr("Log files (*.*)"));
    if (path.isEmpty())
        return;
...
void LogFlux::launchSettingsWindow()
{
    ...
    // Server restart
    connect(settings.ui.btnRestartServer, &QPushButton::clicked, this,
            [this, &settings]()
            {
                startServer(settings.ui.lineEditHost->text(),
...
void LogFlux::setupShortcuts()
{
    ...
    new QShortcut(Qt::Key_G, this, [this]() { goToStartOfLog(); });

    new QShortcut(QKeySequence(Qt::SHIFT | Qt::Key_G), this,
                  [this]()
                  {
                      goToEndOfLog();
                      tail(true);
    ...
    new QShortcut(Qt::Key_E, this, [this]() { goToNextError(); });

    new QShortcut(QKeySequence(Qt::SHIFT | Qt::Key_E), this, [this]() { goToPreviousError(); });

    new QShortcut(Qt::Key_W, this, [this]() { goToNextWarning(); });

    new QShortcut(QKeySequence(Qt::SHIFT | Qt::Key_W), this, [this]() { goToPreviousWarning(); });
    ...
    new QShortcut(QKeySequence(Qt::SHIFT | Qt::Key_Space), this,
                  [this]()
                  {
                      QScrollBar* sb = ui.plainTextEdit->verticalScrollBar();
                      if (sb)
    ...
    new QShortcut(Qt::Key_N, this, [this]() { findNext(); });

    new QShortcut(QKeySequence(Qt::SHIFT | Qt::Key_N), this, [this]() { findPrevious(); });

    // Bookmarks:
    new QShortcut(Qt::Key_M, this, [this]() { addBookmark(); });

    new QShortcut(Qt::Key_B, this, [this]() { goToNextBookmark(); });

    new QShortcut(QKeySequence(Qt::SHIFT | Qt::Key_B), this, [this]() { goToPreviousBookmark(); });
}
...
// Apply quick filters (error/warning OR) by rebuilding the visible document from the buffered
// lines.
...
void LogFlux::onQuickFiltersChanged(bool /*checked*/)
{
    ...
    else
    {
        ...
        {
            m_useQuickFilters = false;
            // Rebuild view with no normal filters (filters were disabled when quick filters were
            // activated). Keep current m_filters empty; just call filtersChanged({}) so logic is
            // consistent.
            filtersChanged({});
        }
    }
